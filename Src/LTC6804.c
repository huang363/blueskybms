...
void LTC_startadc(){
	...
	//3
	wakeup_sleep (); //This will guarantee that the LTC6804 SPI port is awake. This command can be removed.
...
void LTC_readReg(uint8_t reg,uint8_t total_ic,uint8_t *data){
	...
	wakeup_sleep (); //This will guarantee that the LTC6804 isoSPI port is awake. This command can be removed.
...
void run_command(uint32_t cmd)
{
	if(cmd == 1){
		...
		LTC6804_writeReg(TOTAL_IC,tx_cfg);
		while (1){
...
void print_cells(){
	...
	for (int current_ic = 0 ; current_ic < TOTAL_IC; current_ic++){
		prinf(" IC ");
		prinf(current_ic+1);	//in decimal
		for(int i=0; i<12; i++)
		{
			prinf(" C");
			prinf(i+1);	//in decimal
			prinf(":");
			prinf(cell_codes[current_ic][i]*0.0001,4);
			prinf(",");
		}
		prinf();
	}
...
void print_rxconfig()
{
	prinf("Received Configuration ");
	for (int current_ic=0; current_ic<TOTAL_IC; current_ic++)
	{
		prinf(" IC ");
		prinf(current_ic+1,DEC);
		prinft(": 0x");
